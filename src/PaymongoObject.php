...
/**
 * Class PaymongoObject.
 */
class PaymongoObject implements \ArrayAccess, \Countable, \JsonSerializable
{
    ...
    public function __set($k, $v)
    {
        ...
        $this->_values[$k] = util\Util::convertToPaymongoObject($v, $this->_opts);
    ...
    public function &__get($k)
    {
        ...
        if (!empty($this->_transientValues) && $this->_transientValues->includes($k)) {
            ...
            $message = "PayMongo Notice: Undefined property of {$class} instance: {$k}. "
                    . "HINT: The {$k} attribute was set in the past, however. "
                    . 'It was then wiped when refreshing the object '
                    . "with the result returned by PayMongo's API, "
        ...
        $class = \get_class($this);        
        Craft::warning("PayMongo Notice: Undefined property of {$class} instance: {$k}");
        return $nullval;
    }
    ...
    public function refreshFrom($values, $opts, $partial = false)
    {
        ...
        if ($values instanceof PaymongoObject) {
    ...
    {
        foreach ($values as $k => $v) {
            // Special-case metadata to always be cast as a PaymongoObject
            // This is necessary in case metadata is empty, as PHP arrays do
            // not differentiate between lists and hashes, and we consider
            // empty arrays to be lists.
            if (('metadata' === $k) && (\is_array($v))) {
                $this->_values[$k] = PaymongoObject::constructFrom($v, $opts);
            } else {
                $this->_values[$k] = util\Util::convertToPaymongoObject($v, $opts);
            }
            if ($dirty) {
                $this->dirtyValue($this->_values[$k]);
            }
    ...
    public function serializeParameters($force = false)
    {
        ...
        foreach ($this->_values as $k => $v) {
            ...
            //   3. Its value is a PaymongoObject. A PaymongoObject may contain modified
            //      values within in that its parent PaymongoObject doesn't know about.
            //
            $original = \array_key_exists($k, $this->_originalValues) ? $this->_originalValues[$k] : null;
            $unsaved = $this->_unsavedValues->includes($k);
            if ($force || $unsaved || $v instanceof PaymongoObject) {
    ...
    public function serializeParamsValue($value, $original, $unsaved, $force, $key = null)
    {
        ...
        if (\is_array($value)) {
            if (util\Util::isList($value)) {
                ...
                return util\Util::convertToPaymongoObject($value, $this->_opts)->serializeParameters();
            }
        } elseif ($value instanceof PaymongoObject) {
    ...
    /**
     * Returns a pretty JSON representation of the PayMongo object.
     *
     * @return string the JSON representation of the PayMongo object
     */
    ...
    /**
     * Sets all keys within the PaymongoObject as unsaved so that they will be
     * included with an update when `serializeParameters` is called. This
     * method is also recursive, so any PaymongoObject contained as values or
    ...
    protected static function deepCopy($obj)
    {
        ...
        if ($obj instanceof PaymongoObject) {
    ...
    public static function emptyValues($obj)
    {
        if (\is_array($obj)) {
            $values = $obj;
        } elseif ($obj instanceof PaymongoObject) {
            $values = $obj->_values;
        } else {
            throw new Exception\InvalidArgumentException(
    ...
    /**
     * @return null|ApiResponse The last response from the PayMongo API
     */
    ...
    /**
     * Sets the last response from the PayMongo API.
     *
