...
use Craft;
use craft\commerce\paymongo\events\SaveInvoiceEvent;
use craft\commerce\paymongo\models\Invoice;
use craft\commerce\paymongo\records\Invoice as InvoiceRecord;
use craft\db\Query;
...
class Invoices extends Component
{
     ...
     *     $paymongoInvoiceId = $e->invoice->invoiceId;
    ...
    public function saveInvoice(Invoice $invoice): bool
    {
        if ($invoice->id) {
            ...
            if (!$record) {
                throw new Exception(Craft::t('commerce-paymongo', 'No invoice exists with the ID “{id}”', ['id' => $invoice->id]));
            }
        } else {
    ...
    private function _createInvoiceQuery(): Query
    {
        return (new Query())
            ...
            ->from(['{{%paymongo_invoices}} invoices']);
    }
