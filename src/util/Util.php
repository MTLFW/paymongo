...
use craft\commerce\paymongo\util\ObjectTypes;
...
abstract class Util
{
    ...
    /**
     * Converts a response from the PayMongo API to the corresponding PHP object.
     *
     * @param array $resp the response from the PayMongo API
     * @param array $opts
     *
     * @return array|PaymongoObject
     */
    public static function convertToPaymongoObject($resp, $opts)
    { 
        ...
        if (self::isList($resp)) {
            $mapped = [];
            foreach ($resp as $i) {
                \array_push($mapped, self::convertToPaymongoObject($i, $opts));
            }
